...
class Weather extends React.Component {
  ...
  getWeather = async (lat, lon) => {
    ...
    this.updateIcon(this.state.main);
  };

  getWeatherByCity = async (city) => {
    const api_call = await fetch(`${apiKeys.base}weather?q=${city}&units=metric&APPID=${apiKeys.key}`);
    const data = await api_call.json();
    this.setState({
      lat: data.coord.lat,
      lon: data.coord.lon,
      ...
      return (
        <React.Fragment>
          ...
          <Forcast
            ...
            onCitySearch={this.getWeatherByCity}
          />
        </React.Fragment>
      );
    } else {
